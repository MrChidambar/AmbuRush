...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // These routes are specifically for the Android driver app
  
  // Driver login (separate from web login to ensure role validation)
  app.post("/api/driver/login", async (req, res) => {
    try {
      ...
      // Verify password (using your existing auth system)
      ...
      res.json({
        ...
        role: user.role,
        token: `driver_${user.id}_${Date.now()}` // Simple token for demo
      });
    } catch (error) {
  ...
  app.put("/api/driver/location", async (req, res) => {
    try {
      ...
      // Extract driver ID from token (you can implement proper JWT later)
      ...
      if (!ambulance) {
        return res.status(404).json({ message: "No ambulance assigned to driver" });
      }
  ...
  app.get("/api/driver/bookings/active", async (req, res) => {
    try {
      const authHeader = req.headers.authorization;
      if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ message: "Unauthorized" });
      ...
      const ambulance = await storage.getAmbulanceByDriverId(driverId);
      if (!ambulance) {
        return res.json([]); // No ambulance assigned
      }
  ...
  app.put("/api/driver/bookings/:id/status", async (req, res) => {
    try {
      const bookingId = parseInt(req.params.id);
